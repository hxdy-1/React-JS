...
export default function WelcomePage() {
	...
	const opacityOfCity = useTransform(
		scrollY,
		[0, 200, 300, 600],
		[1, 0.5, 0.3, 0]
	);
	const yOfCity = useTransform(scrollY, [0, 200], [0, -100]);

	const opacityOfHero = useTransform(scrollY, [0, 300, 500], [1, 0.8, 0]);
	const yOfHero = useTransform(scrollY, [0, 200], [0, -150]);

	const scaleHeroText = useTransform(scrollY, [0, 300], [1, 1.2]);
	const yOfHeroText = useTransform(scrollY, [0, 200, 300], [0, 100, 150]);

	return (
		<>
			<header id="welcome-header">
				<motion.div
					style={{ scale: scaleHeroText, y: yOfHeroText }}
				...
				<motion.img
					style={{ opacity: opacityOfCity, y: yOfCity }}
					src={cityImg}
					alt="A city skyline touched by sunlight"
					id="city-image"
				/>
				<motion.img
					style={{ opacity: opacityOfHero, y: yOfHero }}
					src={heroImg}
